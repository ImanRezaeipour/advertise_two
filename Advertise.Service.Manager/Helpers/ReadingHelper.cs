...
namespace Advertise.Core.Helpers
{
    public static class CalculateReadingTime
    {
        ...
        {
            var formatted = $"{(span.Duration().Days > 0 ? $"{span.Days:0} روز و " : string.Empty)}{(span.Duration().Hours > 0 ? $"{span.Hours:0} ساعت و " : string.Empty)}{(span.Duration().Minutes > 0 ? $"{span.Minutes:0} دقیقه و " : string.Empty)}{(span.Duration().Seconds > 0 ? $"{span.Seconds:0} ثانیه" : string.Empty)}";
