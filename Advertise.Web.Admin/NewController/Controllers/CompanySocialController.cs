...
namespace Advertise.Web.Controllers
{
    ...
    public partial class CompanySocialController : BaseController
    {
        ...
        [ValidateAntiForgeryToken]
        [MvcAuthorize(PermissionConst.CanCompanySocialEdit)]
        public virtual async Task<ActionResult> Edit(CompanySocialEditViewModel viewModel)
        {
            if (viewModel == null)
                return View(MVC.Error.Views.BadRequest);
            ...
            if (!ModelState.IsValid)
            {
                var viewModelPrepare = await _companySocialFactory.PrepareEditViewModelAsync(viewModel.Id);
                this.MessageError("خطایی بوجود آمده دوباره امتحان کنید");
            ...
            this.MessageSuccess("عملیات با موفقیت انجام شد");
            return RedirectToAction(MVC.CompanySocial.List());
        }


        [MvcAuthorize(PermissionConst.CanCompanySocialList)]
        [MvcSiteMapNode(ParentKey = "Panel_Home_Index", Title = "شبکه های اجتماعی", Key = "Panel_CompanySocial_List")]
        public virtual async Task<ActionResult> List(CompanySocialSearchRequest request)
        {
            var viewModel = await _companySocialFactory.PrepareListViewModelAsync(request);
            return View(MVC.CompanySocial.Views.List, viewModel);
        }
