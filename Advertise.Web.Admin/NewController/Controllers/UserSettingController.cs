...
namespace Advertise.Web.Controllers
{
    ...
    public partial class UserSettingController : Controller
    {
        ...
        public virtual async Task<ActionResult> Edit(UserSettingEditViewModel viewModel)
        {
            if (viewModel == null)
                return View(MVC.Error.Views.BadRequest);
           ...
           await _userSettingService.EditByViewModelAsync(viewModel);
            ...
            return RedirectToAction(MVC.UserSetting.List());
        }


        [MvcSiteMapNode(ParentKey = "Panel_Home_Index", Title = "تنظیمات کاربران", Key = "Panel_UserSetting_List")]
        [MvcAuthorize(PermissionConst.CanUserSettingList)]
        public virtual async Task<ActionResult> List(UserSettingSearchRequest request)
        {
            var viewModel = await _userSettingFactory.PrepareListViewModelAsync(request);
            return View(MVC.UserSetting.Views.List, viewModel);
        }
